$(function() {
    ...
    var current			= -1;
    //cache some elements
    var $btn_thumbs = $('#fullpage_thumbnailtoggle');
    var $loader		= $('#fullpage_loading');
    var $btn_next		= $('#fullpage_next');
    var $btn_prev		= $('#fullpage_previous');
    var $thumbnailScroller	= $('#thumbnailScroller');
    
    //total number of thumbs
    var nmb_thumbs		= $thumbnailScroller.find('.content').length;
    
    //preload thumbs
    var cnt_thumbs 		= 0;
    for(var i=0;i<nmb_thumbs;++i){
        var $thumb = $thumbnailScroller.find('.content:nth-child('+parseInt(i+1)+')');
        $('<img/>').load(function(){
            ++cnt_thumbs;
    ...
    //clicking on a thumb...
    $thumbnailScroller.find('.content').bind('click',function(e){
        var $content= $(this);
        ...
        function(){
            ...
            $('<img class="fullpage_preview"/>').load(function(){
                var $newimg 		= $(this);
                var $currImage 	= $('#fullpage_gallery').children('img:first');
    ...
    function hideThumbs(){
        $('#parent_container').stop().animate({'bottom':'-160px'},500);
        showThumbsBtn();
    }

    function showThumbs(speed){
        $('#parent_container').stop().animate({'bottom':'0px'},speed);
        hideThumbsBtn();
    }
    ...
    function showNext(){
        ++current;
        var $e_next	= $thumbnailScroller.find('.content:nth-child('+current+')');
        if($e_next.length == 0){
            current = 1;
            $e_next	= $thumbnailScroller.find('.content:nth-child('+current+')');
        }
        $loader.show();
        $('<img class="fullpage_preview"/>').load(function(){
            var $newimg 		= $(this);
